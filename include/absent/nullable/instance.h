...
namespace rvarago::absent::nullable::instance {
    ...
    struct binder final {
        static constexpr auto _(Nullable<A, Rest...> const& input, Mapper fn) noexcept -> decltype(fn(std::declval<A>())) {
            using ResultT = decltype(fn(std::declval<A>()));
            if (syntax::empty<Nullable, A, Rest...>::_(input)) {
                return syntax::make_empty<Nullable<A, Rest...>, ResultT>::_(input);
            }
            auto const value = syntax::value<Nullable, A, Rest...>::_(input);
            return fn(value);
        }
    };
    ...
    struct fmapper final {
        static constexpr auto _(Nullable<A, Rest...> const& input, Mapper fn) noexcept -> Nullable<decltype(fn(std::declval<A>())), Rest...> {
            using ValueT = decltype(fn(std::declval<A>()));
            if (syntax::empty<Nullable, A, Rest...>::_(input)) {
                return syntax::make_empty<Nullable<A, Rest...>, Nullable<ValueT, Rest...>>::_(input);
            }
            auto const value = syntax::value<Nullable, A, Rest...>::_(input);
            return syntax::make<Nullable, ValueT, Rest...>::_(fn(value));
        }
    };

...
namespace rvarago::absent {
    ...
    constexpr decltype(auto) bind(Nullable<A, Rest...> input, Mapper fn) noexcept {
        return nullable::instance::binder<Nullable, Mapper, A, Rest...>::_(std::move(input), fn);
    }
    ...
    constexpr decltype(auto) bind(Nullable<A, Rest...> input, member::Mapper<const A, Nullable<B, Rest...>> fn) noexcept {
     ...
     */
    template <template <typename...> typename Nullable, typename A, typename B, typename... Rest>
    constexpr decltype(auto) operator>>(Nullable<A, Rest...> input, member::Mapper<const A, Nullable<B, Rest...>> fn) noexcept {
        return bind(std::move(input), fn);
    }

...
namespace rvarago::absent {
    ...
    constexpr auto bind(Nullable<A> input, Mapper fn) -> decltype(fn(std::declval<A>())) {
        return syntax::nullable<Nullable, Mapper, A>::bind(std::move(input), fn);
    }
    ...
    constexpr auto bind(Nullable<A> input, syntax::member::Mapper<const A, Nullable<B>> fn) -> Nullable<B> {
     ...
     */
    template <template <typename> typename Nullable, typename A, typename B>
    constexpr auto operator>>(Nullable<A> input, syntax::member::Mapper<const A, Nullable<B>> fn) -> Nullable<B> {
        return bind(std::move(input), fn);
    }
